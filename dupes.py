...
def process_files_for_duplication_v5(clean_file_folder, dupes_folder, extras_folder):
    ...
    for file in os.listdir(clean_file_folder):
        if file.endswith('.xlsx'):
            ...
            # Asegurarse de que las columnas necesarias existen antes de intentar ordenar
            if {'BUYBOX SCORE', 'LIKELY DEAL SCORE', 'SCORE'}.issubset(unique_properties.columns):
                unique_properties = unique_properties.sort_values(by=['SCORE', 'LIKELY DEAL SCORE', 'BUYBOX SCORE'], ascending=False)
